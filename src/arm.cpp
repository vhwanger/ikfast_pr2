...
bool Arm::computeIK(const geometry_msgs::PoseStamped &pose, std::vector<double> jnt_pos, std::vector<double> &solution)
{
  ...
  request.ik_request.ik_seed_state.joint_state.position = jnt_pos;
...
void Arm::getCurrentArmConfiguration(vector<double>& current_angles)
{
  ...
  pr2_controllers_msgs::JointTrajectoryControllerStateConstPtr state_msg = ros::topic::waitForMessage<pr2_controllers_msgs::JointTrajectoryControllerState>(controller_state_name_, nh_, ros::Duration(2.0));
  ROS_DEBUG("got it");
